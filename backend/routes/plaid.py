...
@plaid_bp.route('/cleanup-duplicates', methods=['POST'])
@jwt_required()
def cleanup_duplicates():
    """
    One-time cleanup endpoint to remove duplicate transactions and PlaidItems.
    ...
    try:
        ...
        results = {
            ...
            'plaid_items_deleted': 0
        }

        # Step 1: Clean up duplicate PlaidItems for this user
        plaid_items = PlaidItem.query.filter_by(user_id=int(user_id)).order_by(PlaidItem.created_at.desc()).all()

        if len(plaid_items) > 1:
            to_keep = plaid_items[0]  # Keep most recent
            to_delete = plaid_items[1:]  # Delete older ones

            print(f"[CLEANUP] User {user_id}: Keeping most recent PlaidItem (item_id: {to_keep.item_id})")
            print(f"[CLEANUP] Deleting {len(to_delete)} old PlaidItem(s)")

            for item in to_delete:
                print(f"[CLEANUP] - Deleting item_id: {item.item_id}")
                db.session.delete(item)
                results['plaid_items_deleted'] += 1

        # Step 2: Clean up duplicate transactions for this user
        # Find all Plaid transactions grouped by plaid_transaction_id
        duplicate_groups = db.session.query(
            ...
            db.func.count(Transaction.id) > 1
        ).all()

        for txn_id, _ in duplicate_groups:
            # Get all duplicates, keep the first (oldest by created_at)
            duplicates = Transaction.query.filter_by(
                user_id=int(user_id),
            ...
            print(f"[CLEANUP] Found {len(to_delete)} duplicate(s) for transaction {txn_id}")
            print(f"[CLEANUP] Keeping: {to_keep.description} (${to_keep.amount}) - ID: {to_keep.id}")

            for dup in to_delete:
                print(f"[CLEANUP] Deleting duplicate: ID {dup.id}")
                db.session.delete(dup)
                results['transactions_deleted'] += 1

        # Commit all changes
        db.session.commit()

        print(f"[CLEANUP SUCCESS] Deleted {results['plaid_items_deleted']} PlaidItems and {results['transactions_deleted']} transactions")

        return jsonify({
            'message': 'Cleanup completed successfully',
            'plaid_items_deleted': results['plaid_items_deleted'],
            'transactions_deleted': results['transactions_deleted']
        }), 200
